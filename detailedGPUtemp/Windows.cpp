    ...
    if (SUCCEEDED(hres)) {
        ...
        if (SUCCEEDED(hres)) {
            ...
            if (SUCCEEDED(hres)) {
                ...
                hres = pSvc->ExecQuery(bstr_t("WQL"), bstr_t("SELECT * FROM Win32_VideoController"), WBEM_FLAG_FORWARD_ONLY, nullptr, &pEnumerator);
                ...
                if (SUCCEEDED(hres)) {
                    ...
                    while (pEnumerator) {
                        ...
                        if (SUCCEEDED(hr)) {
                            int temperature = vtProp.intVal;
                            std::wcout << L"GPU Temperature: " << temperature << L"°C" << std::endl;
                            VariantClear(&vtProp);
                        }
                        pclsObj->Release();
                    }
